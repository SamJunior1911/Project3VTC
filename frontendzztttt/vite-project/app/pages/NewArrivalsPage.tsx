    ...
    useEffect(() => {
        const fetchBooks = async () => {
            try {
                ...
                // ✅ So sánh ngày chính xác: 7 ngày trước (chỉ tính ngày)
                const now = new Date();
                const sevenDaysAgo = new Date();
                sevenDaysAgo.setDate(now.getDate() - 7);
                sevenDaysAgo.setHours(0, 0, 0, 0);

                const newBooks = allProducts
                    .filter((item: any) => {
                        const createdAt = new Date(item.created_at);
                        createdAt.setHours(0, 0, 0, 0);
                        return createdAt >= sevenDaysAgo && item.status === "active";
                    })
                    .sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

                const processedBooks = await Promise.all(
                    newBooks.map(async (item: any) => {
                        ...
                        try {
                            ...
                            console.warn(`Không lấy được ảnh cho ${item._id}`);
                        }
                        return {
                            id: item._id,
                            title: item.title || "Sách mới về",
                            price: item.price || 0,
                ...
                console.error("Lỗi tải sách mới về:", error);
            } finally {
                setLoading(false);
            }
        };
            ...
            <main className="max-w-7xl mx-auto px-4 py-12">
                ...
                {currentBooks.length === 0 ? (
                    <div className="text-center py-12 bg-white rounded-xl shadow-sm">
                        <h3 className="text-xl font-semibold text-gray-700">Không có sách mới nào trong 7 ngày qua</h3>
                                    ...
                                    >
                                        ...
                                        <div className="relative bg-gray-50 pt-[125%] overflow-visible">
                                            ...
                                            <div className="absolute -top-3 -right-3 z-30 pointer-events-none">
                                                <div className="relative bg-gradient-to-br from-emerald-600 to-teal-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-xl border border-emerald-300 flex items-center gap-1">
                                                    <Clock size={12} />
                                                    <span>MỚI</span>
                                                </div>
                                            </div>
